...
- [x] Write tests for `success(message, data=None)` method in both rich and JSON formats
  - Rich: Assert output contains "[green]✓[/green]" and message
  - JSON: Assert output is valid JSON with {"status": "success", "message": ..., "data": ...}
- [x] Write tests for `error(message, details=None, exit_code=1)` method in both formats
  - Rich: Assert output contains "[red]✗[/red]" and message
  - JSON: Assert output is valid JSON with {"status": "error", ...}
  - Assert: sys.exit() called with correct exit_code
- [x] Write tests for `info(message)`, `warning(message)`, `debug(message)` methods
  - Rich: Assert correct icon ([blue]ℹ[/blue], [yellow]⚠[/yellow], [dim]Debug:[/dim])
  - JSON: Assert {"level": "info|warning|debug", "message": ...}
- [x] Write tests for `progress(description, total)` context manager (rich only)
  - Rich: Assert description printed on entry
  - JSON: Assert no output (silent)
- [x] Write tests for `table(headers, rows)` method (rich table, JSON array)
  - Rich: Assert rich.table.Table rendered
  - JSON: Assert {"type": "table", "headers": [...], "rows": [...]}
- [x] Run tests: `pytest tests/unit/cli/test_output.py -v` - all fail ✓

### Phase 2: Implementation (GREEN)

**Add 7 NEW methods to OutputFormatter class in `src/gitstory/cli/output.py`:**

- [x] Implement `success(message: str, data: dict | None = None) -> None` method
  - Print directly to stdout (rich) or stdout (JSON)
  - Rich: `console.print(f"[green]✓[/green] {message}")`
  - JSON: `print(json.dumps({"status": "success", "message": message, "data": data}))`
- [x] Implement `error(message: str, details: dict | None = None, exit_code: int = 1) -> None` method
  - Print error and call `sys.exit(exit_code)`
  - Rich: `console.print(f"[red]✗[/red] {message}", style="bold red")`
  - JSON: `print(json.dumps({"status": "error", "message": message, "details": details, "exit_code": exit_code}))`
- [x] Implement `info(message: str) -> None`, `warning(message: str) -> None`, `debug(message: str) -> None` methods
  - Print directly to stdout
  - Rich: Uses icons ([blue]ℹ[/blue], [yellow]⚠[/yellow], [dim]Debug:[/dim])
  - JSON: `print(json.dumps({"level": "info|warning|debug", "message": message}))`
- [x] Implement `progress(description: str, total: int)` context manager
...
- [x] Implement `table(headers: list[str], rows: list[list[str]]) -> None` method
  - Print directly to stdout
  - Rich: Create and print `rich.table.Table` with headers and rows
  - JSON: `print(json.dumps({"type": "table", "headers": headers, "rows": rows}))`
- [x] Run tests: `pytest tests/unit/cli/test_output.py -v` - all pass ✓

### Phase 3: Integrate with Commands

- [x] Update plan.py to use OutputFormatter methods
  ...
  - Replace: `console.print(f"[blue]ℹ[/blue] Planning {ticket_id}...")` → `output.info(f"Planning {ticket_id}...")`
  - Replace: `console.print("[yellow]⚠[/yellow] Coming in EPIC-0001.2...")` → `output.warning("Coming in EPIC-0001.2: Workflow engine & planning logic")`
- [x] Update review.py to use OutputFormatter methods
  - Same pattern as plan.py (import, get json_mode, create formatter, replace console.print)
- [x] Update execute.py to use OutputFormatter methods
  - Same pattern as plan.py
- [x] Update validate.py to use OutputFormatter methods
  - Same pattern as plan.py
- [x] Update test_plugin.py to use OutputFormatter methods
  - Same pattern as plan.py
- [x] Update init.py to use OutputFormatter methods
  - Same pattern as plan.py
- [x] Verify: All commands support --json flag
...
- [x] `gitstory plan STORY-0001.2.4` shows rich colored output
- [x] `gitstory --json plan STORY-0001.2.4` outputs valid JSON
- [x] Error messages include exit codes and structured details
- [x] Tables render correctly in both modes
- [x] Progress indicators work in rich mode, silent in JSON mode
- [x] JSON output parseable by `jq` and `python -m json.tool`
- [x] Code coverage >90%
...
# [green]✓[/green] Story created successfully
...
# [red]✗[/red] Invalid ticket ID format
# Provided: INVALID
# (exits with code 1)

# Info/Warning/Debug
output.info("Processing ticket...")        # [blue]ℹ[/blue] Processing ticket...
output.warning("Coming in EPIC-0001.2...")  # [yellow]⚠[/yellow] Coming in EPIC-0001.2...
...
- TASK-0001.1.3.1 complete ✅ (OutputFormatter class exists at src/gitstory/cli/output.py)
- TASK-0001.1.3.2 complete ✅ (6 commands exist: plan, review, execute, validate, test_plugin, init)
- Global --json flag already implemented in cli/__init__.py (ctx.obj["json_mode"])

## Success Criteria

- ✅ OutputFormatter supports all message types (success, error, info, warning, debug)
- ✅ Rich mode renders with colors, icons, and formatting
- ✅ JSON mode outputs valid, parseable JSON
- ✅ Error responses include exit codes and structured details
- ✅ Tables render correctly in both modes
- ✅ Progress indicators work in rich mode, silent in JSON
- ✅ All commands use OutputFormatter consistently
- ✅ Code coverage >90%
